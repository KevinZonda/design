...
const componentText: Record<string, DocTranslation> = {
  ...
  'cookie-banner': { summary: '征求用户设置非必要 Cookie 的许可。', whenToUse: '服务使用分析或其他非必要 Cookie 时使用。等待用户选择期间不要阻止其访问服务。', howItWorks: '示例用 React 本地状态记录选择，并向应用提供接受和拒绝回调。' },
  ...
  'exit-this-page': { summary: '为用户提供醒目的入口，快速离开敏感服务。', whenToUse: '在被他人看到可能使用户面临风险的页面使用。说明该控件无法清除浏览器历史记录。', howItWorks: '链接采用紧急操作样式，应指向中性且加载迅速的目标页面。' },
  ...
  'generic-footer': { summary: '在页面底部添加辅助链接和机构信息。', whenToUse: '适用于 GOV.UK 之外、不需要王室版权或 GOV.UK 许可文字的服务。', howItWorks: 'Footer 提供布局；请提供适合本机构的链接和文字。FooterGovUk 添加标准 GOV.UK 文案。' },
  ...
  footer: { summary: '在页面底部显示版权、许可和辅助导航。', whenToUse: 'GOV.UK 服务应使用标准页脚，并保持核心帮助链接一致。', howItWorks: '导航区块与元信息链接通过数组提供，服务可以只加入相关目标。' },
  header: { summary: '在页面顶部显示 GOV.UK 标识和可选产品名称。', whenToUse: '用于 GOV.UK 服务顶部。服务名称和服务级链接应放在独立的 Service navigation 组件中。', howItWorks: '6.5.1 版页眉渲染官方 GOV.UK 皇冠和文字标志，并可显示产品名称；服务导航独立呈现。' },
  'inset-text': { summary: '将简短的补充信息与周围内容区分开。', whenToUse: '谨慎用于与附近内容相关、但并非主要信息的文字。', howItWorks: '粗左边框和间距带来强调效果，同时不暗示成功、警告或错误状态。' },
  'language-navigation': { summary: '让用户在服务提供的语言之间切换。', whenToUse: '同一服务流程提供两种或更多语言时使用。', howItWorks: '当前语言通过 aria-current 标明，语言导航也有独立的无障碍标签。' },
