...
export const componentDocs: ComponentDoc[] = [
  ...
  },
  {
    ...
    code: `<Breadcrumbs items={[\n  { label: 'Home', href: '#' },\n  { label: 'Passports, travel and living abroad', href: '#' },\n  { label: 'Travel abroad', current: true }\n]} />`,
...
<div className="button-examples__variants">
  <Button disabled style={kss('mb0')}>Disabled button</Button>
  <Button variant="warning" style={kss('mb0')}>Delete account</Button>
  <div className="button-examples__inverse">
    <Button variant="inverse" style={kss('mb0')}>Create an account</Button>
  </div>
</div>
</div>`,
    example: () => <div className="button-examples"><Button href="#start" startButton>Start now</Button><ButtonGroup><Button htmlType="submit">Save and continue</Button><Button variant="secondary">Save as draft</Button><Link href="#cancel">Cancel</Link></ButtonGroup><div className="button-examples__variants"><Button disabled style={kss('mb0')}>Disabled button</Button><Button variant="warning" style={kss('mb0')}>Delete account</Button><div className="button-examples__inverse"><Button variant="inverse" style={kss('mb0')}>Create an account</Button></div></div></div>,
  ...
  },
  {
    ...
    code: `<CharacterCount label="Can you provide more detail?" hint="Do not include personal information." maxLength={200} />`,
  ...
  },
  {
    ...
    api: [{ name: 'options', type: 'ChoiceOption[]', description: 'Checkbox labels, values, hints, conditional content and an exclusive option.' }, { name: 'value', type: 'string[]', description: 'Controlled selected values.' }, { name: 'onChange', type: '(value: string[]) => void', description: 'Called with all selected values.' }, text('legend', 'Question shown as the fieldset legend.'), { name: 'exclusiveDividerText', type: 'string', defaultValue: 'or', description: 'Divider text rendered before an exclusive option.' }, { name: 'ref', type: 'Ref<HTMLFieldSetElement>', description: 'Group fieldset.' }, { name: 'inputRefs', type: 'Record<string, Ref<HTMLInputElement>>', description: 'Refs for individual options, keyed by option value.' }, { name: 'inputProps', type: 'InputHTMLAttributes<HTMLInputElement>', description: 'Native event handlers and attributes shared by each checkbox.' }],
  },
  {
    ...
    code: `<CookieBanner cookieName="kvzd_docs_cookie_demo">\n  We use some essential cookies to make this service work. We would also like to use analytics cookies.\n</CookieBanner>`,
  ...
  },
  {
    ...
    howItWorks: 'The three numeric inputs are grouped in a fieldset with one legend. Their order can change, while the value remains a day, month and year object.',
    code: `<DateInput order={dateOrder.CHN} legend="What is your date of birth?" hint="For example, 1980 03 31" namePrefix="dob" />`,
    example: () => <DateInput order={dateOrder.CHN} legend="What is your date of birth?" hint="For example, 1980 03 31" namePrefix="dob" />,
    api: [text('legend', 'Question for the complete date.'), { name: 'order', type: 'DateInputOrder', defaultValue: 'dateOrder.DMY', description: 'Visible field order: DMY, MDY or YMD. Country aliases GBR, USA and CHN map to these orders.' }, { name: 'value', type: '{ day?: string; month?: string; year?: string }', description: 'Controlled date value.' }, onChange('Called with the complete date value object.'), { name: 'ref', type: 'Ref<HTMLFieldSetElement>', description: 'Group fieldset.' }, { name: 'inputRefs', type: '{ day?, month?, year? }', description: 'Refs for the individual date fields.' }, { name: 'inputProps', type: 'InputHTMLAttributes<HTMLInputElement>', description: 'Native event handlers and attributes shared by the day, month and year fields.' }],
  },
  {
  ...
  },
  {
    ...
    code: `<ErrorSummary errors={[\n  { children: 'Enter your full name', href: '#full-name' },\n  { children: 'The date your passport was issued must be in the past', href: '#passport-date' }\n]} />`,
  ...
  },
  {
    ...
    code: `<FileUpload label="Upload a file" hint="PDF, JPG or PNG up to 10MB" name="evidence" accept=".pdf,.jpg,.jpeg,.png" />`,
  ...
  },
  {
    ...
    code: `<Footer meta={[{ label: 'Help', href: '#' }, { label: 'Privacy', href: '#' }]} description="Service information" copyright="© Example organisation" />`,
  ...
  },
  {
    ...
    code: `<LanguageNavigation items={[{ label: 'English', lang: 'en', href: '#english', current: true }, { label: 'Cymraeg', lang: 'cy', href: '#cymraeg' }]} />`,
  ...
  },
  {
    ...
    code: `<Pagination current={2} total={4} />`,
...
<Panel variant="interruption" title="Is your age correct?" actions={<ButtonGroup><Button href="#continue" variant="inverse">Yes, this is correct</Button><Link href="#change" inverse>No, change my age</Link></ButtonGroup>}>
  ...
  },
  {
    ...
    code: `<div className="search-input-examples">\n  <SearchInput label="Search services" name="query" placeholder="Enter a service" />\n  <SearchInput label="Filter results" iconPosition="right" placeholder="Filter by keyword" styles={{ root: kss('mb0') }} />\n</div>`,
    example: () => <div className="search-input-examples"><SearchInput label="Search services" name="query" placeholder="Enter a service" /><SearchInput label="Filter results" iconPosition="right" placeholder="Filter by keyword" styles={{ root: kss('mb0') }} /></div>,
  ...
  },
  {
    ...
    api: [text('label', 'Select label.'), { name: 'options', type: 'Option[]', description: 'Labels and values.' }, text('error', 'Validation error.'), value(), { name: 'multiple', type: 'boolean', defaultValue: 'false', description: 'Render a multi-select listbox; value and onChange use arrays of selected values.' }, { name: 'value / onChange', type: 'string / (value: string) => void', description: 'Controlled single-select value and change callback.' }, { name: 'showSearch', type: 'boolean', defaultValue: 'false', description: 'Filter the options with a search field. Renders the custom dropdown in single mode.' }, { name: 'loading', type: 'boolean', defaultValue: 'false', description: 'Show a loading state in place of the options. Custom dropdown modes only.' }, { name: 'hiddenInput', type: 'boolean', defaultValue: 'true', description: 'With a name, render hidden inputs so the native form submits the selected value.' }],
  },
  {
  ...
  },
  {
    ...
    code: `<SummaryList items={[\n  { key: 'Name', value: 'Sarah Philips', actions: [{ label: 'Change', href: '#', visuallyHiddenText: 'name' }] },\n  { key: 'Date of birth', value: '5 January 1978', actions: [{ label: 'Change', href: '#', visuallyHiddenText: 'date of birth' }] },\n  { key: 'Address', value: <>72 Guild Street<br />London<br />SE23 6FH</>, actions: [{ label: 'Change', href: '#', visuallyHiddenText: 'address' }] }\n]} />`,
...
<Tabs items={[
  ...
  },
  {
    ...
    code: `<TaskList items={[\n  { title: 'Company details', href: '#', status: 'Completed', statusColor: 'green' },\n  { title: 'Director information', href: '#', status: 'Incomplete', statusColor: 'blue' },\n  { title: 'Check and submit', href: '#', status: 'Cannot start yet' }\n]} />`,
  ...
  },
  {
    ...
    api: [text('label', 'Visible and accessible input label.'), text('hint', 'Optional supporting guidance.'), text('error', 'Validation error.'), { name: 'width', type: '2 | 3 | 4 | 5 | 10 | 20 | 30', description: 'Expected-answer width.' }, { name: 'status', type: `'error' | 'warning'`, description: 'Visual state; error also sets aria-invalid, warning only changes the border colour.' }, { name: 'prefix', type: 'ReactNode', description: 'Content shown before the input, such as a currency symbol.' }, { name: 'suffix', type: 'ReactNode', description: 'Content shown after the input.' }, { name: 'allowClear', type: 'boolean', defaultValue: 'false', description: 'Show a clear button while the input has a value.' }, { name: 'showCount', type: 'boolean | { max?: number }', defaultValue: 'false', description: 'Show a character counter beneath the input, optionally with a maximum.' }],
  },
  {
    ...
    code: `<Textarea label="Can you provide more detail?" hint="Do not include personal or financial information." name="moreDetail" rows={5} />`,
