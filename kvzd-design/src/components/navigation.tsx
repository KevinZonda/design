...
export interface LinkItem extends ClickBehaviourProps { label: ReactNode; current?: boolean }
export const SkipLink = forwardRef<HTMLAnchorElement | HTMLButtonElement | HTMLSpanElement, ClickBehaviourProps & { children?: ReactNode; className?: string; style?: CSSProperties }>(function SkipLink({ href, onClick, children = 'Skip to main content', className = '', style }, ref) {
  return <ClickTarget ref={ref} className={`govuk-skip-link ${className}`.trim()} style={style} href={href ?? (onClick ? undefined : '#main-content')} onClick={onClick}>{children}</ClickTarget>
})

export const BackLink = forwardRef<HTMLAnchorElement | HTMLButtonElement | HTMLSpanElement, ClickBehaviourProps & { children?: ReactNode; className?: string; style?: CSSProperties }>(function BackLink({ href, onClick, children = 'Back', className = '', style }, ref) {
  return <ClickTarget ref={ref} href={href} onClick={onClick} className={`govuk-back-link ${className}`.trim()} style={style}>{children}</ClickTarget>
})

export interface BreadcrumbsProps { items: LinkItem[]; collapseOnMobile?: boolean; className?: string; label?: string; style?: CSSProperties }
...
export interface HeaderProps extends Omit<HTMLAttributes<HTMLElement>, 'title'>, SemanticStyling<'root' | 'container' | 'logo' | 'homeLink'> {
  title: ReactNode
  homeHref?: string
  homeOnClick?: ClickBehaviourProps['onClick']
  logo?: ReactNode
...
export interface ServiceNavigationProps extends SemanticStyling<'root' | 'container'> {
  ...
  serviceUrl?: string
  serviceOnClick?: ClickBehaviourProps['onClick']
  items?: LinkItem[]
  end?: ReactNode
...
export const LanguageNavigation = forwardRef<HTMLElement, { items: (LinkItem & { lang?: string })[]; ariaLabel?: string; className?: string; style?: CSSProperties }>(function LanguageNavigation({ items, ariaLabel = 'Choose language', className = '', style }, ref) {
  return <nav ref={ref} className={`govuk-language-navigation ${className}`.trim()} style={style} aria-label={ariaLabel}><ul className="govuk-language-navigation__list">{items.map((item, index) => {
    const current = item.current || (item.href === undefined && !item.onClick)
...
export interface PaginationLink extends ClickBehaviourProps { text?: ReactNode; label?: ReactNode }
...
function pageWindow(current: number, total: number): Array<number | 'ellipsis'> {
  ...
  for (let page = 1; page <= total; page += 1) {
    ...
    if (pages.length > 0 && page - (pages[pages.length - 1] as number) > 1) pages.push('ellipsis')
    pages.push(page)
  }
  return pages
}
export const Pagination = forwardRef<HTMLElement, PaginationProps>(function Pagination({ current, total, onChange, getHref = (page) => `#page-${page}`, previous, next, previousText = 'Previous', nextText = 'Next', className = '', label = 'Pagination', style }, ref) {
  const block = current === undefined || total === undefined
  if (block) return <nav ref={ref} className={`govuk-pagination govuk-pagination--block ${className}`.trim()} style={style} aria-label={label}>{previous && <PaginationBlockLink direction="prev" link={previous} fallbackText={previousText} />}{next && <PaginationBlockLink direction="next" link={next} fallbackText={nextText} />}</nav>
  ...
  return <nav ref={ref} className={`govuk-pagination ${className}`.trim()} style={style} aria-label={label}>
    {current > 1 && <div className="govuk-pagination__prev"><a className="govuk-link govuk-pagination__link" href={getHref(current - 1)} rel="prev" onClick={handlePage(current - 1)}><PaginationArrow direction="prev" /><span className="govuk-pagination__link-title">{previousText}<span className="govuk-visually-hidden"> page</span></span></a></div>}
    <ul className="govuk-pagination__list">{pages.map((page, index) => page === 'ellipsis'
      ? <li key={`ellipsis-${index}`} className="govuk-pagination__item govuk-pagination__item--ellipses" aria-hidden="true">…</li>
      : <li key={page} className={`govuk-pagination__item ${page === current ? 'govuk-pagination__item--current' : ''}`}><a className="govuk-link govuk-pagination__link" href={getHref(page)} aria-label={`Page ${page}`} aria-current={page === current ? 'page' : undefined} onClick={handlePage(page)}>{page}</a></li>)}</ul>
    {current < total && <div className="govuk-pagination__next"><a className="govuk-link govuk-pagination__link" href={getHref(current + 1)} rel="next" onClick={handlePage(current + 1)}><span className="govuk-pagination__link-title">{nextText}<span className="govuk-visually-hidden"> page</span></span><PaginationArrow direction="next" /></a></div>}
  </nav>
})
