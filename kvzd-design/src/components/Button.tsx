...
export const Button = forwardRef<HTMLAnchorElement | HTMLButtonElement, ButtonProps>(function Button({ children, className = '', href, htmlType = 'button', icon, iconPosition = 'left', loading = false, preventDoubleClick = false, startButton = false, variant = 'primary', ...props }, ref) {
  ...
  const content = <>{leading}{startButton ? <span>{children}</span> : children}{trailing}{startButton && <StartIcon />}</>
  ...
  if (typeof href === 'string') {
    ...
    const linkClasses = `${classesFor(variantClass, startButton, className, inert)}`.trim()
  ...
  return <button {...buttonProps} {...busy} ref={ref as Ref<HTMLButtonElement>} type={htmlType} className={classesFor(variantClass, startButton, className, false)} disabled={inert} aria-disabled={inert ? true : buttonProps['aria-disabled']} onClick={loading ? (event: MouseEvent<HTMLButtonElement>) => { event.preventDefault() } : handleClick}>{content}</button>
}) as {
...
function classesFor(variantClass: string, startButton: boolean, className: string, disabled: boolean) {
  return `govuk-button ${variantClass} ${startButton ? 'govuk-button--start' : ''} ${disabled ? 'govuk-button--disabled' : ''} ${className}`.trim()
}
