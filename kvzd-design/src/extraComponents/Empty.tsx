...
export const Empty = forwardRef<HTMLDivElement, EmptyProps>(function Empty({ title, illustration, actions, children, className = '', classNames, style, styles, ...props }, ref) {
  return <div {...props} ref={ref} className={`kvzd-design-empty ${classNames?.root ?? ''} ${className}`.trim()} style={{ ...styles?.root, ...style }}>
    ...
    {title !== undefined && <h3 className={`govuk-heading-s kvzd-design-empty__title ${classNames?.title ?? ''}`.trim()} style={styles?.title}>{title}</h3>}
