...
import type { ChangeEvent, FieldsetHTMLAttributes, InputHTMLAttributes, ReactNode, Ref, SelectHTMLAttributes, TextareaHTMLAttributes } from 'react'

export const Hint = forwardRef<HTMLDivElement, { id?: string; children: ReactNode }>(function Hint({ id, children }, ref) { return <div ref={ref} id={id} className="govuk-hint">{children}</div> })
export const Label = forwardRef<HTMLLabelElement, { htmlFor?: string; children: ReactNode; size?: 's' | 'm' | 'l' | 'xl' }>(function Label({ htmlFor, children, size }, ref) { return <label ref={ref} className={`govuk-label ${size ? `govuk-label--${size}` : ''}`} htmlFor={htmlFor}>{children}</label> })
export const ErrorMessage = forwardRef<HTMLParagraphElement, { id?: string; children: ReactNode; visuallyHiddenText?: string }>(function ErrorMessage({ id, children, visuallyHiddenText = 'Error:' }, ref) { return <p ref={ref} id={id} className="govuk-error-message"><span className="govuk-visually-hidden">{visuallyHiddenText}</span> {children}</p> })
...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(function Textarea({ className = '', error, hint, id, label, rows = 5, ...props }, ref) {
  ...
  return <div className={`govuk-form-group ${error ? 'govuk-form-group--error' : ''}`}><Label htmlFor={fieldId} size="m">{label}</Label>{hint && <Hint id={`${fieldId}-hint`}>{hint}</Hint>}{error && <ErrorMessage id={`${fieldId}-error`}>{error}</ErrorMessage>}<textarea {...props} ref={ref} id={fieldId} rows={rows} aria-describedby={described || undefined} aria-invalid={error ? true : undefined} className={`govuk-textarea ${error ? 'govuk-textarea--error' : ''} ${className}`.trim()} /></div>
})
...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(function Select({ className = '', error, hint, id, label, options, placeholder, ...props }, ref) {
  ...
  return <div className={`govuk-form-group ${error ? 'govuk-form-group--error' : ''}`}><Label htmlFor={fieldId} size="m">{label}</Label>{hint && <Hint id={`${fieldId}-hint`}>{hint}</Hint>}{error && <ErrorMessage id={`${fieldId}-error`}>{error}</ErrorMessage>}<select {...props} ref={ref} id={fieldId} aria-describedby={described || undefined} className={`govuk-select ${error ? 'govuk-select--error' : ''} ${className}`.trim()}>{placeholder && <option value="">{placeholder}</option>}{options.map((option) => <option key={option.value} value={option.value} disabled={option.disabled}>{option.label}</option>)}</select></div>
})

export interface ChoiceOption extends Option { hint?: ReactNode; conditional?: ReactNode }
export interface ChoiceGroupProps { name: string; legend: ReactNode; options: ChoiceOption[]; value?: string[]; defaultValue?: string[]; onChange?: (value: string[]) => void; hint?: ReactNode; error?: ReactNode; small?: boolean; inputRefs?: Record<string, Ref<HTMLInputElement>>; inputProps?: Omit<InputHTMLAttributes<HTMLInputElement>, 'checked' | 'value' | 'defaultChecked' | 'defaultValue' | 'onChange' | 'name' | 'type' | 'id'> }
export const Checkboxes = forwardRef<HTMLFieldSetElement, ChoiceGroupProps>(function Checkboxes({ defaultValue = [], error, hint, inputProps, inputRefs, legend, name, onChange, options, small = false, value }, ref) {
...
export const FileUpload = forwardRef<HTMLInputElement, FileUploadProps>(function FileUpload({ className = '', error, hint, id, label, ...props }, ref) {
  const uid = useId(); const fieldId = id ?? `kvzd-upload-${uid.replaceAll(':', '')}`
  return <div className={`govuk-form-group ${error ? 'govuk-form-group--error' : ''}`}><Label htmlFor={fieldId} size="m">{label}</Label>{hint && <Hint>{hint}</Hint>}{error && <ErrorMessage>{error}</ErrorMessage>}<input {...props} ref={ref} id={fieldId} type="file" className={`govuk-file-upload ${error ? 'govuk-file-upload--error' : ''} ${className}`.trim()} /></div>
})

export interface CharacterCountProps extends Omit<TextareaProps, 'maxLength'> { maxLength?: number; maxWords?: number }
export const CharacterCount = forwardRef<HTMLTextAreaElement, CharacterCountProps>(function CharacterCount({ maxLength = 200, maxWords, onChange, value, defaultValue, ...props }, ref) {
  ...
  return <div className="govuk-character-count" data-module="govuk-character-count"><Textarea {...props} ref={ref} value={text} onChange={(event) => { if (value === undefined) setInner(event.target.value); onChange?.(event) }} /><div className={`govuk-hint govuk-character-count__message ${remaining < 0 ? 'govuk-error-message' : ''}`} aria-live="polite">You have {Math.abs(remaining)} {maxWords ? 'word' : 'character'}{Math.abs(remaining) === 1 ? '' : 's'} {remaining < 0 ? 'too many' : 'remaining'}</div></div>
})

export interface PasswordInputProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'type'> { label?: ReactNode; showText?: string; hideText?: string; onVisibilityChange?: (visible: boolean) => void }
export const PasswordInput = forwardRef<HTMLInputElement, PasswordInputProps>(function PasswordInput({ className = '', label = 'Password', showText = 'Show', hideText = 'Hide', onVisibilityChange, ...props }, ref) {
  const [visible, setVisible] = useState(false); const uid = useId(); const id = props.id ?? `kvzd-password-${uid.replaceAll(':', '')}`
  return <div className="govuk-form-group govuk-password-input"><Label htmlFor={id}>{label}</Label><div className="govuk-input__wrapper govuk-password-input__wrapper"><input {...props} ref={ref} id={id} className={`govuk-input govuk-password-input__input ${className}`.trim()} type={visible ? 'text' : 'password'} spellCheck={false} autoCapitalize="none" /><button className="govuk-button govuk-button--secondary govuk-password-input__toggle" type="button" aria-controls={id} aria-label={`${visible ? hideText : showText} password`} aria-pressed={visible} onClick={() => { const next = !visible; setVisible(next); onVisibilityChange?.(next) }}>{visible ? hideText : showText}</button></div></div>
})
