...
export function Input({ className = '', error, hint, id, label, labelSize, status, width, ...props }: InputProps) {
  ...
  return (
    <div className={`govuk-form-group ${hasError ? 'govuk-form-group--error' : ''}`}>
      ...
      <input {...props} id={inputId} aria-describedby={describedBy || undefined} aria-invalid={hasError || undefined} className={`govuk-input ${hasError ? 'govuk-input--error' : ''} ${width ? `govuk-input--width-${width}` : ''} ${className}`.trim()} />
    </div>
  )
}
